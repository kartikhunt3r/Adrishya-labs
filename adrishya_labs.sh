...
██─██▀██─▀─███─▄─▀█▄▄▄▄─█
▀▄▄▄▄▄▀▄▄▀▄▄▀▄▄▄▄▀▀▄▄▄▄▄▀
                                               ${BIYellow} @kartikhunt3r
"""
    echo
    echo -e "${BGreen}Usage: $0 {list|status|info|start|startpublic|stop} [projectname]" >&2
    echo
    ...
    echo
    echo -e "${BIBlue} Dockerfiles from:"
...
#    echo "  Security Sheperd       - OWASP Project (ismisepaul/securityshepherd)"
    ...
    echo -e "  Vulnerable GraphQL API - Carve Systems LLC (carvesystems/vulnerable-graphql-api) ${NC}"
...
if ! [ -x "$(command -v docker)" ]; then
  echo 
  echo -e "${BIRed}Docker was not found. Please install docker before running this script. ${NC}"
...
then 
	echo -e "${BIRed}Docker is not running. ${NC}"
	echo -n "Do you want to start docker now (y/n)?"
	read answer
	if echo "$answer" | grep -iq "^y"; then
		sudo service docker start
	else
		echo -e "${BIRed}Not starting. Script will not be able to run applications. ${NC}"
	fi
fi
...
#########################
list() {
    echo -e "${BIBlue}Available pentest applications" >&2
    ...
    echo -e "  graphql		- Vulnerable GraphQL API${NC}"
...
function addhost() { # ex.   127.5.0.1	bwapp
    ...
    if [ -n "$(grep $2 /etc/hosts)" ]
        then
            ...
            if [ -n "$(grep $2 /etc/hosts)" ]
                then
                    echo -e "${BGreen}$HOSTS_LINE was added succesfully to /etc/hosts${NC}";
                else
                    echo "${BRed}Failed to Add $2, Try again!${NC}";
            fi
    fi
}
